"""
pipeline.py  —  GhostStore Personal pipeline
compress → chunk → encrypt → embed → manifest → vault
...
def hide_v2(
    ...
    carrier_type='image',       # 'image' | 'video' | 'audio'
    ...
    print(f'  ✂️  Split into {len(raw_chunks)} chunk(s) × up to {chunk_size//1024} KB')
...
def reveal_v2(
    ...
    compressed = reassemble(dec_chunks)
